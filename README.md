# Human Action Recognition — NTU60 skeleton and video inference
...
│   ├── test_mmpose_image.py            # detector + COCO-17 image contract
│   ├── extract_video_pose.py           # video -> dominant-person pose pickle
│   ├── infer_action_joint.py           # frozen Joint-only acceptance step
│   ├── demo_video.py                   # windowed fusion + annotated MP4
│   ├── test_domain_gap.py              # >=5 labeled real-video checks
│   └── report_video_inference.py       # measured final milestone report
├── src/inference/                      # pose, preprocessing, action, rendering
├── hooks/
...
│   └── 06_video_mmpose_stgcn.ipynb    # end-to-end offline video inference
├── artifacts/
...
Run `notebooks/06_video_mmpose_stgcn.ipynb` in a fresh Kaggle GPU session. It
does not need `ntu60_2d.pkl` and never invokes a training entry point. Attach
exactly one copy of each frozen checkpoint:

```text
stgcn_joint_ntu60_xsub_best.pth
stgcn_joint_motion_ntu60_xsub_best.pth
```

Also attach one MP4 named `demo_input.mp4` (or set `DEMO_VIDEO` in the first
cell). Internet must be enabled on first use so TorchVision and MMPose can
download their official detector and pose weights. For the complete domain-gap
acceptance test, attach at least five labeled clips plus one
`domain_gap_manifest.csv`:

```csv
video_path,expected_action
clapping.mp4,clapping
sitting_down.mp4,sitting down
standing_up.mp4,standing up (from sitting position)
reading.mp4,reading
writing.mp4,writing
```

Relative video paths are resolved beside the manifest, and each action must
exactly match an NTU60 class name. The notebook deliberately stops rather than
fabricating the five-video report when those inputs are absent.

The inspected input contract is documented in
`artifacts/inference/training_preprocessing.md`. Pixel-space COCO-17 keypoints
and confidence pass through the official MMAction2 v1.2.0 validation
transforms. Motion is generated by `GenSkeFeat(feats=['jm'])` before temporal
sampling. Each model receives `(1, 1, 2, 100, 17, 3)`. Long videos use
100-frame windows with 50-frame overlap, frozen `0.5 / 0.5` score fusion, and
probability-space EMA smoothing.

MMPose 1.3.2 eagerly imports optional EDPose and RTMO registries, which require
compiled MMCV ops and MMDetection respectively. The notebook records and
applies a pinned, fail-closed compatibility patch that disables only those two
unused heads. The selected official MobileNetV2 + HeatmapHead COCO model is
unchanged, and the full remaining registry is imported as an acceptance check.
This avoids replacing the working `mmcv-lite`/MMAction2 environment.

Successful completion writes:

```text
artifacts/inference/example_pose.pkl
artifacts/inference/mmpose_lite_patch.json
artifacts/inference/preprocessing_verification.json
artifacts/inference/window_predictions.json
artifacts/inference/latency_report.json
artifacts/inference/domain_gap_test.csv
artifacts/inference/video_inference_report.md
artifacts/demo/demo_output.mp4
```

The first implementation supports one dominant person and intentionally has
no tracker. Missing detections remain zero-confidence pose frames.

## Out of scope (current milestone)

Further Joint training, Bone/Bone-Motion training, ST-GCN++, YOLO, ByteTrack,
RGB/skeleton fusion, live webcam inference, TensorRT/ONNX export, NTU120,
Kinetics400, and hyperparameter tuning. MMPose is used only for this offline
pose-extraction path; none of the listed follow-up work is started.
